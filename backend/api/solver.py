...
def get_min_workers_both_weeks(multiplier: int) -> Dict[int, int]:
    ...
    Returns:
        Dict[int, int]: A dictionary mapping day indices (1-42) to the minimum number of workers required.

    Notes:
        - Days 1 to 21 correspond to the first week, while days 22 to 42 correspond to the second week.
...
def get_min_workers_second_week(multiplier: int) -> Dict[int, int]:
    ...
    Returns:
        Dict[int, int]: A dictionary mapping day indices (22-42) to the minimum number of workers required.
...
def reoptimize_schedule_after_sickness(
    ...
    Args:
        ...
        day_index (int): The specific day (1-based index) up to which the schedule should be optimized.
    ...
    for current_week_roster, next_week_roster in zip(current_week_rosters, next_week_rosters):
        ...
        schedules[username] = get_roster_mapping(
            current_week_roster.schedule + next_week_roster.schedule, 1, ROSTER_INDEX_END)
        work_days[username] = get_day_mapping(
            current_week_roster.work_days + next_week_roster.work_days, FIRST_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
        off_days_roster_current_week = merge_roster_strings(
            current_week_roster.off_days, current_week_roster.reserve_call_in_days, current_week_roster.day_off_call_in_days)
        off_days_roster_next_week = merge_roster_strings(
            next_week_roster.off_days, next_week_roster.reserve_call_in_days, next_week_roster.day_off_call_in_days)
        off_days[username] = get_day_mapping(
            off_days_roster_current_week + off_days_roster_next_week, FIRST_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
        reserve_days[username] = get_day_mapping(
            current_week_roster.reserve_days + next_week_roster.reserve_days, FIRST_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
        vacation[username] = get_day_mapping(
            current_week_roster.vacation + next_week_roster.vacation, FIRST_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
        sickness[username] = get_day_mapping(
            current_week_roster.sickness + next_week_roster.sickness, FIRST_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
    ...
    for worker in WORKERS:
        ...
        for day in DAYS_INDEX_1_14:
            var_workDays[worker, day] = solver.BoolVar(
                f'var_workDays[{worker},{day}]')
            var_offDays[worker, day] = solver.BoolVar(
                f'var_offDays[{worker},{day}]')
            var_reserve[worker, day] = solver.BoolVar(
                f'var_reserve[{worker},{day}]')
            ...
            p_workDays[worker, day] = solver.BoolVar(
                f'p_workDays[{worker},{day}]')
            p_offDays[worker, day] = solver.BoolVar(
                f'p_offDays[{worker},{day}]')
    ...
    for worker in WORKERS:
        ...
        for day in DAYS_INDEX_1_14:
            ...
            objective.SetCoefficient(var_workDays[worker, day], coefficient)

            coefficient = float(off_days[worker][day])
            objective.SetCoefficient(var_offDays[worker, day], coefficient)

            coefficient = float(reserve_days[worker][day])
            objective.SetCoefficient(var_reserve[worker, day], coefficient)

            objective.SetCoefficient(p_workDays[worker, day], e)
            objective.SetCoefficient(p_offDays[worker, day], f)
    ...
    for worker in WORKERS:
        ...
        if day_index > 7:
            # Define work days
            solver.Add(sum((var_workDays[worker, day] + p_workDays[worker, day])
                       for day in DAYS_INDEX_8_14) >= sw_number_of_work_days)

            # Define off days
            solver.Add(sum((var_offDays[worker, day] + p_offDays[worker, day])
                       for day in DAYS_INDEX_8_14) >= sw_number_of_off_days)

            # Define reserve days
            solver.Add(sum((var_reserve[worker, day] + p_reserve[worker, day])
            ...
            for day in range(day_index, 15):
                solver.Add(sum(var_reserve[worker, day]
        ...
        # Cut on first week
            ...
            solver.Add(sum(var_workDays[worker, day]
                       for day in DAYS_INDEX_8_14) == sw_number_of_work_days)

            # Define off days
            solver.Add(sum((var_offDays[worker, day] + p_offDays[worker, day])
                       for day in DAYS_INDEX_1_7) >= fw_number_of_off_days)
            solver.Add(sum(var_offDays[worker, day]
                       for day in DAYS_INDEX_8_14) == sw_number_of_off_days)
        ...
        for day in DAYS_INDEX_1_14:
            solver.Add(var_workDays[worker, day] + var_offDays[worker, day] +
                       var_reserve[worker, day] + var_vacation[worker, day] + var_sickness[worker, day] == 1)

        # Each worker can only have shifts on workdays
        for day in DAYS_INDEX_1_14:
            solver.Add(sum(var_schedule[worker, (day - 1) * 3 + k]
                       for k in range(1, 4)) == var_workDays[worker, day])
        ...
        if fw_number_of_off_days > 0 and fw_max_cnsc_num_vac_sick_days > 1 and not fw_reserve_call_in[worker]:
            # Ensure a reserve day follows a day off
            for day in DAYS_INDEX_1_6:
                solver.Add(var_offDays[worker, day + 1]
                           >= var_reserve[worker, day])

            # Ensure a reserve day cannot be preceded by an off day
            for day in DAYS_INDEX_2_7:
                solver.Add(var_offDays[worker, day - 1]
                           <= 1 - var_reserve[worker, day])

            solver.Add(sum(var_schedule[worker, (day - 1) * 3 + 3]
                       for day in DAYS_INDEX_1_7) <= 2)

        if sw_number_of_off_days > 0 and sw_max_cnsc_num_vac_sick_days > 1 and not sw_reserve_call_in[worker]:
            # Ensure a reserve day follows a day off
    ...
    if status == pywraplp.Solver.OPTIMAL:
        for worker in WORKERS:
            ...
            if day_index > 7:
                ...
                for day in DAYS_INDEX_8_14:
                    res_work_days += str(
                        int(var_workDays[worker, day].solution_value()))
                    res_off_days += str(
                        int(var_offDays[worker, day].solution_value()))
                    res_reserve_days += str(
                        int(var_reserve[worker, day].solution_value()))
...
def optimize_schedule(number_of_users_to_solve: int, multiplier: float) -> Tuple[int, int, int]:
    ...
    Returns:
        Tuple[int, int, int]: A tuple containing:
            - `status` (int): The solver's status (e.g., `pywraplp.Solver.OPTIMAL` for a successful solution).
    ...
    for next_week_roster, application_week_roster in zip(next_week_rosters, application_week_rosters):
        username = application_week_roster.owner.username
        WORKERS.append(username)

        application_week_applications[username] = get_roster_mapping(
            application_week_roster.application, ROSTER_INDEX_START, ROSTER_INDEX_END)
        next_week_schedules[username] = get_roster_mapping(
            next_week_roster.schedule, ROSTER_INDEX_START, ROSTER_INDEX_END, ROSTER_INDEX_START - 1)
        next_week_work_days[username] = get_day_mapping(
            next_week_roster.work_days, FIRST_WEEK_DAY_INDEX_START, FIRST_WEEK_DAY_INDEX_END)
        next_week_off_days[username] = get_day_mapping(
            next_week_roster.off_days, FIRST_WEEK_DAY_INDEX_START, FIRST_WEEK_DAY_INDEX_END)
        next_week_reserve_days[username] = get_day_mapping(
            next_week_roster.reserve_days, FIRST_WEEK_DAY_INDEX_START, FIRST_WEEK_DAY_INDEX_END)
        application_week_vacation[username] = get_day_mapping(
            application_week_roster.vacation, SECOND_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
        application_week_sickness[username] = get_day_mapping(
            application_week_roster.sickness, SECOND_WEEK_DAY_INDEX_START, SECOND_WEEK_DAY_INDEX_END)
    ...
    for worker in WORKERS:
        ...
        for day in DAYS_INDEX_8_14:
            if int(application_week_vacation[worker][day]) == 1 or int(application_week_sickness[worker][day]) == 1:
                var_workDays[worker, day].SetBounds(0, 0)
                var_offDays[worker, day].SetBounds(0, 0)
                var_reserve[worker, day].SetBounds(0, 0)
    ...
    if status == pywraplp.Solver.OPTIMAL:
        for worker in WORKERS:
            ...
            for day in DAYS_INDEX_8_14:
                res_work_days += str(
                    int(var_workDays[worker, day].solution_value()))
                res_off_days += str(int(var_offDays[worker,
                                    day].solution_value()))
                res_reserve_days += str(
                    int(var_reserve[worker, day].solution_value()))
